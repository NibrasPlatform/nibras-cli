...
function writeCheckConfig(dir, { topLevel = {}, subject = {}, project = {} } = {}) {
  fs.writeFileSync(
...
test('autoCheck grades semantic questions with a mocked grader', async () => {
  ...
  const result = await autoCheck({
    ...
    aiGrader: async () => ({
      ...
      evidenceQuotes: ['later negative edge can lower a distance that was already finalized'],
    }),
  });
...
test('run test treats explicit --grading as strict even when requireGrading is false', async () => {
  ...
  await withWorkingDirectory(dir, () =>
    assert.rejects(
      () => run(['node', 'bin/nibras.js', 'cs161', 'test', 'exam1', '--grading', 'missing.json']),
    ...
    choices: [
      {
        message: {
          content: JSON.stringify({
            ...
            reasoningSummary: 'Partial answer.',
            evidenceQuotes: ['Once you finalize a node, a later negative edge can lower it again.'],
          }),
        },
      },
    ],
