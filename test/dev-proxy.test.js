...
test("dev proxy sends /v1 requests to the API and other requests to the web app", async () => {
  ...
  try {
    ...
    await Promise.all([
      ...
      new Promise((resolve, reject) => api.server.close((error) => error ? reject(error) : resolve())),
      new Promise((resolve, reject) => web.server.close((error) => error ? reject(error) : resolve()))
    ]);
  }
});
